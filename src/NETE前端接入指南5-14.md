...
- `team_perf = subtree_perf`（含本人与下级）
- `big_leg_perf = team_perf - own_perf - small_leg_perf`
...
|---:|---:|---:|---:|---:|---:|
| 0 | 100 | 1 | 35 | 0 | 3000 |
| 1 | 100 | 3 | 35 | 12000 | 2900 |
| 2 | 300 | 3 | 35 | 12000 | 2800 |
| 3 | 500 | 2 | 35 | 12500 | 2700 |
| 4 | 1000 | 1 | 40 | 13000 | 2600 |
| 5 | 3000 | 1 | 40 | 13500 | 2500 |
| 6 | 5000 | 1 | 40 | 13500 | 2400 |
| 7 | 10000 | 1 | 50 | 14000 | 2300 |
| 8 | 30000 | 1 | 50 | 14000 | 2200 |
| 9 | 50000 | 1 | 50 | 14000 | 2000 |


前端接入注意：


- `activateMiner` 仅允许付费档位（`tierIndex` 必须为 `1..9`）；传 `0` 会在链上 `revert InvalidTier()`。
- `tierIndex` 同时决定激活所需本金、同档位可持有上限、周期、总收益比例、提现手续费。
- 当 `presaleActive=true` 时，`tierIndex 1..6` 还会受预售额度限制（售罄会 `revert PresaleTierSoldOut()`）。
...
- `AirdropExpired`：提示“空投矿机已过期，无法继续领取”。
...
- 设 `P=principal`，`R=totalReturn`，`pending=本次可领总额`：
  ...
  - `profitFee = profitGross * feeBps / 10000`
  - `profitNet = profitGross - profitFee`
- 入账：
  - `principalPart -> repurchaseBalance`
  - `profitNet -> positionProfit`
  - `profitFee -> 费分账（Burn/Treasury/Dividend）`
...
- `withdrawProfit` 现在是**纯提现净利润**，不再在提现时扣手续费。
- 手续费已在 claim 分账阶段前置扣除。
...
链上一键复投入口：`NeteCore.repurchaseExpiredMiners()`


该函数只处理“到期且待复投”的付费矿机，前端应先按合约条件筛选，再决定按钮是否可点击。
...
- `eligible.length == 0`：隐藏或置灰，文案 `暂无到期待复投矿机`
- `eligible.length > 0 && repurchaseBalance < required`：置灰，文案 `复投余额不足`
- `repurchase_paused == true`：置灰，文案 `系统暂停复投`
- 其余情况：按钮可点，调用 `repurchaseExpiredMiners()`


错误兜底提示：


- `NoExpiredMinerForRepurchase`：暂无到期待复投矿机
...
### 6.4.1 空投矿机（NFT自助领取并自动激活）


链上入口：


- `NeteCore.claimAndActivateAirdropMiner()`


调用顺序（前端）：


1. 读取是否已配置 NFT 合约（`sbtContract`）并检查用户是否持有 NFT。
2. 若用户符合条件，点击“领取并激活空投矿机”按钮。
3. 发送 `claimAndActivateAirdropMiner()` 交易（单次交易内完成“领取碎片 + 合成激活”）。
4. 交易成功后刷新：
   - `getUserPositions`（仓位列表）
   - `income/overview`
   - `income/ledger`
...
- `NFTRequired`：提示“未持有指定 NFT，无法领取空投矿机”。
- `NFTAlreadyClaimed`：提示“该地址已领取过空投矿机”。
- `AirdropAlreadyComposed`：提示“该地址已存在空投矿机，不能重复激活”。
- `FragmentInsufficient`：提示“碎片不足，无法合成空投矿机”（理论上新入口下通常不出现，保留兜底提示）。
...
- 全量分页：`GET /v1/airdrop/claims?page=1&page_size=20`
- 按用户分页：`GET /v1/airdrop/claims/:user?page=1&page_size=20`
...
- `page`：可选，默认 `1`
- `page_size`：可选，默认 `20`
- `user`（路径参数）：地址查询接口必填，建议前端统一转小写后再传
...
- `user`：领取地址
- `nft_contract`：用于校验领取资格的 NFT 合约地址
- `amount`：本次领取碎片数量（当前固定为 `100`，仍按字符串处理）
- `position_id`：本次自动激活生成的空投矿机仓位 ID
- `tx_hash`：交易哈希
- `log_index`：日志索引（与 `tx_hash` 组合可唯一定位事件）
- `block_number`：区块高度
- `claimed_at`：领取时间（Unix 秒）


前端页面入口建议：


- **我的空投记录**：在“空投矿机”模块增加“领取记录”入口，调用 `GET /v1/airdrop/claims/:user`
- **运营后台（可选）**：在管理后台增加“空投领取总览”，调用 `GET /v1/airdrop/claims`
...
- `GET /v1/airdrop/claims` -> `ApiListResponse<AirdropClaimView>`
- `GET /v1/airdrop/claims/:user` -> `ApiListResponse<AirdropClaimView>`
